...
class AssimilatorBuilder:
        ...
        if self.ai.can_afford(unit.ASSIMILATOR) and self.ai.structures(unit.PYLON).exists:
            for nexus in self.ai.structures(unit.NEXUS):
                ...
                for vaspene in vaspenes:
                    if not self.ai.structures(unit.ASSIMILATOR).exists or not self.ai.structures(
                            unit.ASSIMILATOR).closer_than(5, vaspene).exists:
    ...
    def standard(self):
        if self.ai.structures().filter(
                lambda x: x.type_id in [unit.GATEWAY, unit.WARPGATE] and x.is_ready).amount < 1 or \
                (self.ai.structures(unit.NEXUS).ready.amount > 1 and self.ai.vespene > self.ai.minerals * 2):
            return
        nexuses = self.ai.structures(unit.NEXUS)
        if nexuses.amount < 4:
            nexuses = nexuses.ready
