...
class RapidExpansion(StrategyABS):
    def __init__(self, ai):
        super().__init__(type='macro', name='RapidExpansion', ai=ai)
        ...
        main_division_units = {unit.ZEALOT: 5, unit.STALKER: 5, unit.IMMORTAL: 7, unit.ARCHON: 8, unit.SENTRY: 3,
                               unit.DISRUPTOR: 4, unit.COLOSSUS: 3, unit.OBSERVER: 1}
        self.army.create_division('stalkers1', STALKER_x5, [stalker_micro], Movements(ai, 0.6), lifetime=-200)
        self.army.create_division('adepts', {unit.ADEPT: 2}, [adept_micro], Movements(ai, 0.2))
        ...
        self.army.create_division('observer', OBSERVER_x1, [ObserverMicro(ai)], Movements(ai, 0.2), lifetime=-800)
        self.army.create_division('warpprism', WARPPRISM_x1, [warpprism_micro], Movements(ai, 0.2), lifetime=-400)

        build_queue = BuildQueues.RAPID_EXPANSION
    ...
    async def nexus_abilities(self):
        self.chronobooster.rapid_expansion()
