...
class MoracleMicro(MicroABS):
    revelation_range = 6
    retired_oracles = 0

    def __init__(self, ai):
        super().__init__('MoracleMicro', ai)
        ...
        self.mineral_lines = [self.ai.mineral_field.closer_than(9, exp).center.towards(exp, -3) for exp in expansions][:7]

        # enemy_position = self.ai.enemy_start_locations[0]
        self.max_dist = min(self.ai.game_info.map_size) - 5
        self.start_micro = False
        ...
        self.oracle_first_position = self.get_safe_position()
    ...
    def get_safe_position(self):
        ...
        position = enemy_expansions[5 if random.uniform(0,1) > 0.5 else 6]
        return self.ai.mineral_field.closer_than(9, position).center.towards(position, -35)
    ...
    async def do_micro(self, division):
        ...
        # if all_oracles.amount >= 4:
        #     self.start_micro = True
        # if not self.start_micro:
        #     return oracles.amount
        multiple_oracles = all_oracles.amount > 1
        for oracle in oracles:
            if not self.oracle_on_harassment and oracle.health_percentage == 1:
                self.oracle_on_harassment = True
            ...
            if self.oracle_on_harassment:
                ...
                #     self.oracle_first_position_visited = False
                shield_min = 0.65 if threats.amount > 2 else 0.45
                ...
                if multiple_oracles and dist < 20 or (not self.ai.enemy_units().exists or
                                                      not self.ai.enemy_units().closer_than(9, oracle).exists or
                      not self.ai.enemy_structures().exists or not self.ai.enemy_structures().closer_than(9,oracle).exists):

                    condition = sum([(oracle2.energy > 45 or ability.BEHAVIOR_PULSARBEAMOFF in
                                await self.ai.get_available_abilities(oracle2)) and oracle2.shield_percentage > shield_min
                                     for oracle2 in all_oracles]) > 0.8 * all_oracles.amount
                ...
                if condition:
                    ...
                    if workers.amount < 1:
                        ...
                        if oracle.distance_to(self.mineral_lines[self.enemy_base_idx].position) < 7:
                            # print('close!')
                            self.enemy_base_idx += 1
                            pos = self.mineral_lines[self.enemy_base_idx].towards(self.oracle_first_position, 3)
                            oracle.move(pos)
                    ...
                    elif workers.amount > 0 or queens.amount > 0:
                    ...
                    if dist > 30 or dist < self.oracle_last_dist:
                        ...
                        if self.set_new_first_pos:
                            ...
                            if y2 == 5:
                                ...
                                y2 = 5
                            self.oracle_first_position = Point2((y2,x2))
                        if oracle.health_percentage < 0.25 and MoracleMicro.retired_oracles < 1:
                            MoracleMicro.retired_oracles += 1
                            self.oracle_on_harassment = False
                            oracle.move(self.oracle_first_position)
                            oracle.move(self.ai.defend_position, queue=True)
                ...
                elif invisible_threats and self.ai.attack or invisible_threats and \
                    invisible_threats.closer_than(40,self.ai.defend_position):
