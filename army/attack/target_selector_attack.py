    ...
    def select_target(self):
        if self.ai.attack:
            ...
            enemy = enemy_units.filter(lambda x: x.type_id not in self.ai.units_to_ignore and not x.is_hallucination
                        and x.cloak != 1 and not x.is_snapshot and x.is_visible and x.type_id not in {unit.OVERLORD,
                        unit.OVERSEER, unit.OVERLORDTRANSPORT} and (x.can_attack_ground or x.can_attack_air))
            ...
            if enemy:
                half_enemy_amount = enemy.amount
                for base in self.ai.townhalls:
                    enemy_near_base = enemy.closer_than(21, base)
                    if enemy_near_base.amount > half_enemy_amount:
                        destination = base.position
                        return destination
                    else:
                        enemy = enemy.further_than(21, base)
