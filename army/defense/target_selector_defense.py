...
class TargetSelectorDefense(TargetSelector):
    ...
    def select_target(self):
        if self.ai.attack:
            ...
            enemy = enemy_units.filter(lambda x: x.type_id not in self.ai.units_to_ignore and not x.is_hallucination
                                                 and x.cloak != 1 and not x.is_snapshot and x.is_visible and x.type_id not in {
                                                 unit.OVERLORD,
                                                 unit.OVERSEER, unit.OVERLORDTRANSPORT} and (
            ...
            if enemy:
                for base in self.ai.townhalls.ready:
                    enemy_near_base = enemy.closer_than(24, base)
                    if enemy_near_base.amount > 1:
                        destination = base.position
                        return destination


            destination = min(self.ai.townhalls.ready,
                                  key=lambda x: sum([x.distance_to(y) for y in self.ai.townhalls.ready]))
        ...
        # else:
        #     destination = self.ai.defend_position
        #
        # return destination