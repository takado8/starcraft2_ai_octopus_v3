...
class OctopusEvo(sc2.BotAI):
    ...
    async def on_step(self, iteration: int):
        ...
        if build_in_progress or build_finished or army_priority or self.counter_attack_condition() or \
                (self.minerals > 500 and self.vespene > 300):
    ...
    def forge_upg_priority(self):
        if self.structures(unit.TWILIGHTCOUNCIL).ready.exists:
            upgds = [upgrade.PROTOSSGROUNDWEAPONSLEVEL1, upgrade.PROTOSSGROUNDARMORSLEVEL1, upgrade.PROTOSSSHIELDSLEVEL1,
                upgrade.PROTOSSGROUNDWEAPONSLEVEL2, upgrade.PROTOSSGROUNDARMORSLEVEL2, upgrade.PROTOSSSHIELDSLEVEL2,
                upgrade.PROTOSSGROUNDWEAPONSLEVEL3, upgrade.PROTOSSGROUNDARMORSLEVEL3, upgrade.PROTOSSSHIELDSLEVEL3]
        else:
        ...
        if not done:
            ...
            if forges.exists:
                if forges.amount < 3:
                    ...
                    max_idle = 2
                if forges.idle.amount > max_idle:
                    return True
        return False
