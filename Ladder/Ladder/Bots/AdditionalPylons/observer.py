...
'''
_debug = False
_max_range = 35  # max range to get to a lurker or widow mine.
...
class Observer:
	...
	def moveToEnemies(self):
		hiddenTarget = self.findHiddenTarget()
		if hiddenTarget:
			self.last_target = hiddenTarget
			if self.checkNewAction('move', hiddenTarget.position[0], hiddenTarget.position[1]):
				self.game.combinedActions.append(self.unit.attack(hiddenTarget))
			return True
		elif self.game.moveToEnemies(self):
			return True
		return False
		


	def findHiddenTarget(self):
		...
		if len(self.game.burrowed_mines) > 0:
			...
			for tag, [position, lastseen] in self.game.burrowed_mines.items():
				...
				if dist < closestDistance:
					#check if another observer is already tracking.
					if not self.game.unitList.hiddenAlreadyTracked(position, self):
						closestDodge = position
						closestDistance = dist
			if closestDistance < _max_range:
				self.hiddenTracker = closestDodge
				return closestDodge
