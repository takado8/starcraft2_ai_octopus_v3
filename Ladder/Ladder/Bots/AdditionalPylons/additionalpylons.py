...
############
	...
	def CooldownKite(self, unit_obj, targetEnemy,  bonus_range=0):
		#kite if we can.
		if targetEnemy:
	...
	def attack(self, unit_obj, bonus_range=0):
		if unit_obj.unit.weapon_cooldown == 0:
			...
			#weapon is on cooldown, see if it's a good idea to kite.
			targetEnemy = self.findBestTarget(unit_obj, bonus_range)
			if targetEnemy:
				#find out if target is facing us.
				if not self.targetFacing(unit_obj, targetEnemy):
					return False
				enemy_range = 0
				enemy_wspeed = 0
				our_range = 0
				our_wspeed = 0
				if unit_obj.unit.is_flying:
					enemy_range = targetEnemy.air_range
					enemy_weapon = next((weapon for weapon in targetEnemy._weapons if weapon.type in TARGET_AIR), None)
					if enemy_weapon:
						enemy_wspeed = enemy_weapon.speed
				else:
					enemy_range = targetEnemy.ground_range
					enemy_weapon = next((weapon for weapon in targetEnemy._weapons if weapon.type in TARGET_GROUND), None)
					if enemy_weapon:
						enemy_wspeed = enemy_weapon.speed
				if targetEnemy.is_flying:
					our_range = unit_obj.unit.air_range
					our_weapon = next((weapon for weapon in unit_obj.unit._weapons if weapon.type in TARGET_AIR), None)
					if our_weapon:
						our_wspeed = our_weapon.speed
				else:
					our_range = unit_obj.unit.ground_range
					our_weapon = next((weapon for weapon in unit_obj.unit._weapons if weapon.type in TARGET_GROUND), None)
					if our_weapon:
						our_wspeed = our_weapon.speed					
				#find out if target has faster firing speed than us.
				if (our_wspeed < enemy_wspeed):
					return False
					
				#find out if target had 1 unit +/- our range.
				if enemy_range < (our_range - 1) or enemy_range > (our_range + 1):
					return False
				
				#looks like we should kite.
				if self.CooldownKite(unit_obj, targetEnemy, bonus_range=bonus_range):
					return True
		return False
	...
	def civilAirDefense(self):
		if self.cached_enemies.of_type([PHOENIX,OVERLORD]):
			return True
		return False
