...
class Phoenix:
	...
	def runList(self):
		...
		our_under_attack = False
		if (self.game.under_attack):
			our_under_attack = True
		else:
			#check if air units exist.
			our_under_attack = self.game.civilAirDefense()
		...
		if self.closestEnemies.amount > 0:		
			...
			if (not self.game.defend_only or our_under_attack) and self.game.moveNearEnemies(self):
		...
		if self.game.defend_only and not our_under_attack:
