...
class UnitList():
	...
	def hiddenAlreadyTracked(self, checked, checking_unit):
		trackList = {k : v for k,v in self.unit_objects.items() if v.unit.name == 'Observer' and v.hiddenTracker == checked and v.unit.tag != checking_unit.unit.tag }
		if len(trackList) > 0:
			return True		
		return False		
